...
- Rust 命令放在 `src-tauri/src/commands/<module>.rs`，并在 `commands/mod.rs` 与 `src-tauri/src/lib.rs` 注册。
- TypeScript 调用封装放在 `src/api/<module>.ts`，再从 `src/api/tauri.ts` 统一导出。
- 页面放在 `src/pages/<Feature>.tsx`，纯工具函数和测试放在 `src/pages/<feature>/`。

文件操作类功能必须遵守：

- 默认不覆盖已有文件。
- 执行前先预览或确认。
- 后端必须重新校验前端传入的路径和目标，不能只信任 UI。
- 涉及批量操作时记录日志，日志模块名使用 `[]` 前缀，例如 `[重命名]`、`[文生图]`。

当前新增模块：

- `FileOrganize.tsx` + `fileOrganize/utils.ts`：文件归类，前端生成日期/类型/扩展名移动预览，执行前检查目标路径是否已存在，Rust 后端执行移动并拒绝覆盖；页面内保留最近一次成功项用于撤销。
- `BatchRename.tsx`：批量重命名，支持前后缀、查找替换、序号、大小写规则；执行前检查目标路径是否已存在并弹确认框，页面内保留最近一次成功项用于撤销。
- `image_generation.rs`：文生图生成与连接自检；默认配置仅作为初始 Base URL/Key 来源，用户可在界面覆盖。
- `LogViewer.tsx`：运行日志查看、搜索、级别筛选、模块筛选和复制。
